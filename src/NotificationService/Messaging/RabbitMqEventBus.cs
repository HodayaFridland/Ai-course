...
/// <summary>
/// Identical RabbitMQ wrapper to the other services'. One durable FANOUT exchange "orders.events";
/// this service owns the "notification.saga" queue. eventType + correlationId ride in the AMQP
/// message properties so the correlation id survives the trip through the broker.
/// </summary>
public class RabbitMqEventBus : IDisposable
{
    ...
    public RabbitMqEventBus(IConfiguration config, ILogger<RabbitMqEventBus> logger)
    {
        ...
        _queueName = config["MessageBroker:Queue"] ?? "notification.saga";
